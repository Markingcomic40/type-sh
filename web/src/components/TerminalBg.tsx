...
const COMMANDS: Command[] = [
  ...
  // Our own take on an agent session, not any real one
  {
    cmd: () => 'npx @typesh/agent',
    lines: () => {
      const file = randPick([
        'src/wpm.ts',
        'src/typing.ts',
      ...
      const ms = 600 + Math.floor(Math.random() * 1800);
      return [
        { txt: '  ◆ agent  v1.2.0', cls: 'accent' },
        { txt: '  ───────────────────────────────────────', cls: 'muted' },
        {
          txt: '  cwd: ~/code/typesh   model: sonnet   mode: edit',
          cls: 'muted',
        },
        '',
        { txt: '› add a test for the wpm oscillator', cls: 'prompt' },
        '',
        { txt: '  • reading ' + file, cls: 'info' },
        { txt: '  • reading src/wpm.test.ts', cls: 'info' },
        { txt: '  ✎ editing src/wpm.test.ts (+18 −0)', cls: 'warn' },
        { txt: '  $ pnpm test --filter wpm', cls: 'muted' },
        { txt: '    ✓ oscillates around the target (4 assertions)', cls: 'ok' },
        {
          txt: '  ✔ done — 1 file changed, tests green in ' + ms + 'ms',
          cls: 'ok',
        },
        '',
        { txt: '› looks good, commit it', cls: 'prompt' },
        '',
        {
          txt: '  $ git add -A && git commit -m "test: cover wpm oscillator"',
          cls: 'muted',
        },
        {
          txt: '  [main ' + randHex(7) + '] test: cover wpm oscillator',
          cls: 'ok',
        },
        '',
        { txt: '› /exit', cls: 'prompt' },
        { txt: '  ◆ session ended · 2 turns · saved transcript', cls: 'muted' },
      ];
    },
  },
